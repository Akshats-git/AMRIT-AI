...
import { deleteFileIndex, getDatabase, getRepoById, getRepoIndexCounts, insertChunks, insertEndpoints, insertSymbols, normalizeFilePath, updateRepoIndexState, upsertRepo, } from "./db.js";
 ...
 */
export function getJavaGlobPattern(repoDir) {
 ...
 */
export async function findJavaFiles(repoDir) {
...
export async function syncRepository() {
    ...
    const changedFiles = diffOutput
        .split("\n")
        .map((line) => normalizeGlobPath(line.trim()))
        .filter(Boolean)
        .filter(isJavaFile);
    return {
        repoDir,
 ...
 */
export function getLineSnippet(lines, lineNumber, radius = 2) {
 ...
 */
export function findEnclosingTypeName(node) {
 ...
 */
export function getAnnotationName(annotationText) {
 ...
 */
export function normalizeEndpointPath(basePath, routePath) {
 ...
 */
export function extractMappingPaths(annotationArguments) {
 ...
 */
export function parseMappingAnnotation(annotationText) {
 ...
 */
export function extractClassBasePaths(node, sourceText) {
 ...
 */
export function extractClassBasePathsFromSource(sourceText) {
 ...
 */
export function parseEndpointsFromMethod(node, sourceText, basePaths, relativeFilePath) {
    const annotations = (node.children ?? []).filter((child) => child.type === "annotation");
 ...
 */
export function extractEndpointsFromSourceWindow(lines, methodLineNumber, controller, methodName, relativeFilePath, basePaths = [""]) {
 ...
 */
export function extractSymbolsAndEndpoints(sourceText, lines, relativeFilePath, tree) {
    const symbols = [];
    const endpoints = [];
    const visit = (node, inheritedBasePaths = [""]) => {
        ...
        if (node.type === "method_declaration" || node.type === "constructor_declaration") {
            ...
            const dedupedEndpoints = new Map();
 ...
 */
export function buildChunks(lines, relativeFilePath) {
    const chunks = [];
 ...
 */
export async function parseJavaFile(repoDir, relativeFilePath) {
 ...
 */
export async function reindexFile(repoDir, filePath) {
